...
// [[Rcpp::export]]
Rcpp::RObject msdpd_me_slm_aqs(const arma::vec& para, 
                        const arma::mat& x_, 
                        const arma::vec& y,
                        const arma::vec& y1, 
                        const arma::sp_mat& w, 
  ...
  const vec beta = (x_.t()*bdinv_c*x_).i()*x_.t()*bdinv_c*(irw*y - theta*y1);
  const vec k_ast = irw*y - x_*beta - theta*y1;
  ...
  vec eq = vec(2);
  if (correction) {
    field<sp_mat> A_mats = make_A_df(p, t, theta*speye(p, p), sp_mat(p,p), iirws);
